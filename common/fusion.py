...
class Fusion(data.Dataset):
    def __init__(self, opt, root_path, train=True, MAE=False):
        ...
        if self.train:
            ...
            self.generator = ChunkedGenerator(opt.batch_size, None, self.poses_train,
                                              self.poses_train_2d, None, chunk_length=1, pad=self.pad,
                                              augment=opt.data_augmentation, reverse_aug=opt.reverse_augmentation,
                                              kps_left=self.kps_left, kps_right=self.kps_right,
                                              joints_left=self.joints_left,
                                              joints_right=self.joints_right, out_all=opt.out_all, MAE=False, train = True)
            ...
            self.generator = ChunkedGenerator(opt.batch_size, None, self.poses_test,
                                              self.poses_test_2d, self.valid_frame, chunk_length=1,
                                              pad=self.pad, augment=False, kps_left=self.kps_left,
                                              kps_right=self.kps_right, joints_left=self.joints_left,
                                              joints_right=self.joints_right, MAE=False, train = False)
    ...
    def prepare_data(self, path, train=True):
        ...
        #     train_dataset = "data_train_univ_3dhp_reproject.npz"
        #     test_dataset = "data_test_univ_3dhp_reproject.npz"
        # else:
        #     train_dataset = "data_train_3dhp.npz"
        #     test_dataset = "data_test_3dhp.npz"

        train_dataset = "/data_train_" + self.dataset + ".npz"
        test_dataset = "/data_test_" + self.dataset + ".npz"

        if train == True:
            data = np.load(path + train_dataset,allow_pickle=True)['data'].item()
            for seq in data.keys():
                cameras = self.cameras if self.cameras is not None else data[seq][0].keys()
                for cam in cameras:
                    cam = str(cam)
                    anim = data[seq][0][cam]
                    ...
                    out_poses_3d[(subject_name, seq_name, cam)] = data_3d / 1000.0
            ...
            data = np.load(path + test_dataset, allow_pickle=True)['data'].item()
            for seq in data.keys():
                ...
                data_3d[:, :14] -= data_3d[:, 14:15]
                data_3d[:, 15:] -= data_3d[:, 14:15]
                out_poses_3d[seq] = data_3d / 1000.0
    ...
    def __getitem__(self, index):
        ...
        # if gt_3D.shape[1] == 1 and input_2D.shape[1] == 1:
        #     gt_3D = gt_3D.squeeze()
        #     input_2D = input_2D.squeeze()
        # else:
        #     raise "2nd dimension not removable"

        if self.train == False and self.test_aug:
            _, gt_3D_aug, input_2D_aug, _, _,_ = self.generator.get_batch(seq_name, start_3d, end_3d, flip=True, reverse=reverse)
            input_2D = np.concatenate((np.expand_dims(input_2D,axis=0),np.expand_dims(input_2D_aug,axis=0)),0)
            gt_3D = np.concatenate((np.expand_dims(gt_3D,axis=0),np.expand_dims(gt_3D_aug,axis=0)),0)
        ...
        if self.train == True:
            return cam, gt_3D.squeeze(), input_2D_update.squeeze(), seq, subject, scale, bb_box, cam_ind
        else:
            return cam, gt_3D.squeeze(), input_2D_update.squeeze(), seq, scale, bb_box, 0, 0
